...
public class MixinFogRenderer {
    ...
    private static Vector3d onSampleColor(Vector3d val) {
        ...
        if (mc.player == null) return val;
        final ClientWorld world = mc.player.clientLevel;

        if (world.dimensionType().hasSkyLight()) {
            return world.getSkyColor(mc.gameRenderer.getMainCamera().getBlockPosition(), mc.getFrameTime());
        } else {
            return val;
        }
    }
