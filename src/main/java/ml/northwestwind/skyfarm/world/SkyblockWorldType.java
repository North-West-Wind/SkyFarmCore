...
public class SkyblockWorldType extends ForgeWorldType {
    ...
    public static SimpleRegistry<Dimension> skyblockDimensions(Registry<DimensionType> dimensionTypes, Registry<Biome> biomeRegistry, Registry<DimensionSettings> settings, long seed) {
        ...
        simpleregistry.register(Dimension.NETHER, new Dimension(() -> {
            return dimensionTypes.getOrThrow(DimensionType.NETHER_LOCATION);
        }, new NoiseChunkGenerator(NetherBiomeProvider.Preset.NETHER.biomeSource(biomeRegistry, seed), seed, () -> {
            return settings.getOrThrow(NETHER_SETTINGS);
        })), Lifecycle.stable());
        simpleregistry.register(Dimension.END, new Dimension(() -> {
            return dimensionTypes.getOrThrow(DimensionType.END_LOCATION);
        }, defaultEndGenerator(biomeRegistry, settings, seed)), Lifecycle.stable());
        return simpleregistry;
    }

    private static ChunkGenerator defaultEndGenerator(Registry<Biome> biomeRegistry, Registry<DimensionSettings> settings, long seed) {
        return new NoiseChunkGenerator(new EndBiomeProvider(biomeRegistry, seed), seed, () -> {
            return settings.getOrThrow(DimensionSettings.END);
        });
    }
}