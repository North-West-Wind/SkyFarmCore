...
public class EvaporatingRecipe implements IRecipe<IInventory> {
    ...
    public EvaporatingRecipe(ResourceLocation id, Ingredient input, ItemStack output, int tick, double chance) {
        this(RegistryEvents.Recipes.EVAPORATING.getType(), id, input, output, tick, chance);
    }

    public EvaporatingRecipe(IRecipeType<?> type, ResourceLocation id, Ingredient input, ItemStack output, int tick, double chance) {
        this.type = type;
        this.id = id;
        this.output = output;
        this.input = input;
